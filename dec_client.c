...
int main(int argc, char* argv[]) {
    ...
    if (charsWritten < strlen(buffer)) {
        error("DEC_CLIENT: WARNING: Not all data written to socket!\n");
        exit(1);
    }
    ...
    // Checks for a valid connection to DEC_SERVER
    if (strcmp(buffer, "@@") != 0) {
        fprintf(stderr, "ENC_CLIENT: ERROR: Can not connect\n"); //Gives weird ": Success!" with error()
        fflush(stderr);
        exit(2);
    }
    ...
    // Print decrypted message to standard out/document
