...
* Parameter: decryptedMsg, key, message
* Output: Decrypted message of the encrypted message given by the user
* Why use this: I used this function to ease the load off my main and so I can easily concert the characters
*               back to their original state.
********************/
char* decryptFunct(char decryptedMsg[], char key[], char message[]) {
    ...
    for (int i = 0; i < strlen(message); i++) {
        ...
        // If it's a negative number add 27
        if (finalChar < 0) finalChar = finalChar + 27;
    ...
    decryptedMsg[counter] = '\n';
    decryptedMsg[counter + 1] = '\0';
...
int main(int argc, char* argv[]) {
    ...
    char decryptedMsg[100000];          //  Holds final decrypted message
    ...
    while (1) {
        ...
        switch (pid) {
            ...
            case 0:
                ...
                // Send Message back to client. @@ for DEC.
                strcat(clientMsg, "@@");
                ...
                // Decrpyt Message + store into decryptedMsg
                strcpy(decryptedMsg, decryptFunct(decryptedMsg, keyBuffer, messageBuffer));

                // Send decryted message to client
                charsRead = send(connectionSocket, decryptedMsg, strlen(decryptedMsg), 0);
                if (charsRead < 0) {
                    error("DEC_SERVER: ERROR writing to socket");
                    childExitMethod = 1;
                    exit(1);
